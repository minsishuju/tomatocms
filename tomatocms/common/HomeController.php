...
class HomeController extends Controller
{
    ...
    public function __construct()
    {
        ...
        define("CMSNAME", $this->config("cmsname") ?: 'TomatoCMS');
        ...
        //安全拦截
        if(! ! $safe_model = Config::get('safemodel'))
        {
            $theurl=$_SERVER["QUERY_STRING"];
            $biaoshi=explode(",",Config::get('url_allow'));
            $urlgo=false;
            foreach ($biaoshi as $bs){
                if(strstr( $theurl,$bs) !== false) {$urlgo=true; break;}
            }
            if(strstr( $theurl,'p=') == false && $urlgo==false  && strlen($theurl)>0){
                _404('您访问的地址有误，请核对后重试！');
            }elseif(strstr( $theurl,'p=') !== false && strstr( $theurl,'&') !== false && $urlgo==false)
            {
                _404('您访问的地址有误，请核对后重试！');
            }
        }
        ...
        if (! ($this->config('wap_domain') && is_mobile()) && (! ! $main_domain = Config::get('main_domain')) && (! ! $to_main_domain = Config::get('to_main_domain'))) {
            if (! preg_match('{^' . $main_domain . '$}i', get_http_host(true))) {
                if (is_https()) {
                    header("Location: https://" . $main_domain . ':' . $_SERVER['SERVER_PORT'].$_SERVER['REQUEST_URI'], true, 301);
                } else {
                    header("Location: http://" . $main_domain . ':' . $_SERVER['SERVER_PORT'].$_SERVER['REQUEST_URI'], true, 301);
                }
                exit();
            }
        }
