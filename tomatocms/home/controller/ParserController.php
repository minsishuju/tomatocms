...
class ParserController extends Controller
{
    ...
    public function parserSingleLabel($content)
    {
        ...
        $content = str_replace('{pboot:islogin}', session('tomato_uid') ? 1 : 0, $content); // 是否登录
        if (strpos($content, '{pboot:mustlogin}') !== false) {
            $content = str_replace('{pboot:mustlogin}', '', $content);
            if (! session('tomato_uid')) { // 没有经登录
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                switch ($matches[1][$i]) {
                    ...
                    case 'logo':
                        ...
                        break;
		    case 'mlogo':
			if (isset($data->mlogo) && $data->mlogo) {
				if (! preg_match('/^http/', $data->mlogo)) {
					$content = str_replace($matches[0][$i], $this->adjustLabelData($params, SITE_DIR . $data->mlogo), $content);
				} else {
					$content = str_replace($matches[0][$i], $this->adjustLabelData($params, $data->mlogo), $content);
				}
			} else {
				$content = str_replace($matches[0][$i], STATIC_DIR . '/images/logo.png', $content);
			}
...
//                                        $order = $value . ',a.istop DESC,a.isrecommend DESC,a.isheadline DESC,a.sorting ASC,a.date DESC,a.id DESC';
                ...
                //如果指定文章id则跳过该id
                if($skipids){
					//先替换字符串中的中文逗号，然后用英文逗号分割为数组
					$content_arr = explode(',', str_replace('，',',',$skipids));
					//经过两次键值翻转进行去重
					$content_arr=array_flip(array_flip($content_arr));
					foreach($content_arr as $v_id)
					{
						if(gettype($v_id)!='integer')
						{
							error('指定跳过的文章id，需要是正整数，请检查contentid 参数:'.$v_id);
						}
					}
					
                    $where3[] = "a.id not in ($skipids)";
                }
                // 起始数校验
        ...
        switch ($label) {
            ...
            case 'content':
                ...
                if (!!$tags = $this->model->getTags()) {
                    ...
                    //实现对被过滤掉的短关键词进行补充，实现长词优先，兼顾短词 短词替换1次
                    if (count($shortkeywords) > 0) {
                        $TomatoTag = new TomatoExtensionController();
                        $data->content = $TomatoTag->ShortKeyWord($shortkeywords, $data->content);
                    }
                }
        ...
        foreach ($params as $key => $value) {
            switch ($key) {
                case 'showgcode': // 指定等级显示，支持多个逗号隔开
                    ...
                    if (! in_array(session('tomato_gcode'), $showgcode)) {
                        return false;
                    }
                    break;
                case 'showucode': // 指定用户显示，支持多个逗号隔开
                    $showucode = explode(',', $params['showucode']);
                    if (! in_array(session('tomato_ucode'), $showucode)) {
                        return false;
                    }
                    break;
                case 'hidegcode': // 指定等级隐藏，支持多个逗号隔开
                    $hidegcode = explode(',', $params['hidegcode']);
                    if (in_array(session('tomato_gcode'), $hidegcode)) {
                        return false;
                    }
                    break;
                case 'hideucode': // 指定用户隐藏，支持多个逗号隔开
                    $hideucode = explode(',', $params['hideucode']);
                    if (in_array(session('tomato_ucode'), $hideucode)) {
                        return false;
                    }
                    break;
                case 'showgcodelt': // 等级小于显示
                    if ($params['showgcodelt'] <= session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'showgcodegt': // 等级大于显示
                    if ($params['showgcodegt'] >= session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'showgcodele': // 等级小于等于显示
                    if ($params['showgcodele'] < session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'showgcodege': // 等级大于等于显示
                    if ($params['showgcodege'] > session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'hidegcodelt': // 等级小于隐藏
                    if ($params['hidegcodelt'] > session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'hidegcodegt': // 等级大于隐藏
                    if ($params['hidegcodegt'] < session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'hidegcodele': // 等级小于等于隐藏
                    if ($params['hidegcodele'] >= session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'hidegcodege': // 等级大于等于隐藏
                    if ($params['hidegcodege'] <= session('tomato_gcode')) {
                        return false;
                    }
                    break;
                case 'showlogin': // 登录后显示
                    if ($params['showlogin'] && ! session('tomato_uid')) {
                        return false;
                    }
                    break;
                case 'hidelogin': // 登录后隐藏
                    if ($params['hidelogin'] && session('tomato_uid')) {
                        return false;
                    }
                    break;
            }
