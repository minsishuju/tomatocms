...
class IndexController extends Controller
{
                ...
                default:
                    ...
                    if ($iscontent) {
                        ...
                        if (!!$data && $suffix && !$err) {
                            ...
                            /*兼容转换前的网址*/
                            if(Config::get('url_jianrong')){
                                header("Location: http://" . $_SERVER['HTTP_HOST'].$this->jianrongurl($_SERVER["QUERY_STRING"]), true, 301);
                            }else{
                                _404('您访问的内容不存在，请核对后重试！');
                            }
    ...
    private function getAboutPage($sort)
    {
        ...
        $content = str_replace('{pboot:pagekeywords}', $sort->keywords ? '{sort:keywords}' : '{content:keywords}', $content);
        $content = str_replace('{pboot:pagedescription}', $sort->description ? '{sort:description}' : '{content:description}', $content);
        $content = $this->parser->parserPositionLabel($content, $sort->scode); // CMS当前位置标签解析
        $content = $this->parser->parserSortLabel($content, $sort); // CMS分类信息标签解析
        $content = $this->parser->parserCurrentContentLabel($content, $sort, $data); // CMS内容标签解析
        $content = $this->parser->parserCommentLabel($content); // 文章评论
    ...
    //兼容旧版url
    private function jianrongurl($url)
    {
        $url=str_replace('p=','',$url);
        if(strstr( $url,'&') !== false)  $url=str_replace('p=','',substr($url,0,strrpos($url ,"&")));
        if(file_exists(ROOT_PATH.DIRECTORY_SEPARATOR.Config::get('url_path'))){
            $jianrong=explode(PHP_EOL,file_get_contents(ROOT_PATH.DIRECTORY_SEPARATOR.Config::get('url_path')));
            $jrurl=[];
            foreach ($jianrong as $k=>$v)
            {
                $biaoshi=explode(' ',$v);
                $biaoshi[0]=ltrim($biaoshi[0],'/');
                $biaoshi[1]=ltrim($biaoshi[1],'/');
                $jrurl[$biaoshi[0]]=$biaoshi[1];
            }
        }
        if(array_key_exists($url,$jrurl))
        {
            return '/'.$jrurl[$url];
        }
        elseif (substr($url,-5)=='.html')
        {
            $data_id=explode('/',str_replace('.html','',$url));
            $data_id=end($data_id); //用 / 将字符串分割为数组，并将最后一个数组元素赋值给$data_id

            $content_data= new ParserModel();
            $urlpath=$content_data->getSort($content_data->getContent($data_id)->scode)->filename;
            if($urlpath==null){ _404('您访问的页面不存在，请核对后重试！');}
            return '/'.$urlpath.'/'.$data_id.'.html';
        }
        else
        {
            _404('您访问的页面不存在，请核对后重试！');
