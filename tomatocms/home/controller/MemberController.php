...
class MemberController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->TomatoMember = new TomatoMemberController();
    }
    ...
    public function login()
    {
        ...
        if ($_POST) {
            ...
            if (! ! $login = $this->model->login("(username='$username' or useremail='$username' or usermobile='$username') AND password='$password'")) {
                ...
                session('tomato_uid', $login->id);
                session('tomato_ucode', $login->ucode);
                session('tomato_username', $login->username);
                session('tomato_useremail', $login->seremail);
                session('tomato_usermobile', $login->usermobile);
                session('tomato_gid', $login->gid);
                session('tomato_gcode', $login->gcode);
                session('tomato_gname', $login->gname);
    ...
    {
        session('tomato_uid', '');
        session('tomato_ucode', '');
        session('tomato_username', '');
        session('tomato_useremail', '');
        session('tomato_usermobile', '');
        session('tomato_gid', '');
        session('tomato_gcode', '');
        session('tomato_gname', '');
        location(Url::home('member/login'));
    }
    ...
    public function sendEmail()
    {
        ...
        if ($to) {
            ...
            $mail_subject = "【" . $this->config('systemtitle') . "】您有新的验证码信息，请注意查收！";
            $code = create_code(4);
    ...
    public function isRegister()
    {
        ...
        if ($this->config('register_type') == 2) { // 邮箱注册
            ...
            if($this->config('black_name') && in_array($username,explode(',',$this->config('black_name')))){
                $err = '用户账号含有保留字符！';
            }
            // 检查用户名
            if ($this->model->checkUsername("username='$username' OR useremail='$username' OR usermobile='$username'")) {
    ...
    {
        $this->TomatoMember->profiles();
    }

    public function profiles2()
    {
        $this->TomatoMember->membernav();
    }
}