...
class ConfigController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($_POST) {
            ...
            switch (post('submit')) {
                case 'accredit':
                    success('修改成功！', url('/admin/Config/index' . get_tab('t101'), false));
                    break;
                case 'custom':
                    success('修改成功！', url('/admin/Config/index' . get_tab('t100'), false));
                    break;
                case 'enclosure':
        ...
        // 获取主题列表
        $themes = dir_list(ROOT_PATH . $this->config('tpl_dir')['admin']);
    ...
    private function modDbConfig($key)
    {
        ...
        {
            if(stripos($value,'//')){
                $value=substr($value, stripos($value,'//')+2);
            }
            if(stripos($value,'/')){
