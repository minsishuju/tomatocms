...
class IndexController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->assign('systemtitle', $this->config('systemtitle'));
        $login_pics=$this->config('login_pics')?: $this->config('tpl_dir')['admin'].'/default/images/bg1.jpg';
        $login_pics='"'.str_replace(',','","',$login_pics).'"';

        $this->assign('login_pics', $login_pics);
    ...
    public function home()
    {
        ...
        if (get_db_type() == 'sqlite') {
            // 数据库配置含有默认名字则进行修改
            if (strpos($this->config('database.dbname'), 'tomatocms') !== false) {
                if (get_user_ip() != '127.0.0.1' && $this->modDB()) { // 非本地测试时尝试自动修改数据库名称
                    $dbsecurity = true;
                } else {
                    $dbsecurity = false;
                }
            } elseif (file_exists(ROOT_PATH . '/data/tomatocms.db')) { // 存在多余的默认数据库文件则改名
                rename(ROOT_PATH . '/data/tomatocms.db', ROOT_PATH . '/data/' . get_uniqid() . '.db');
            }
        } elseif (file_exists(ROOT_PATH . '/data/tomatocms.db')) {
            rename(ROOT_PATH . '/data/tomatocms.db', ROOT_PATH . '/data/' . get_uniqid() . '.db');
        }
        ...
        $this->assign('site', get_http_host());
        ...
        $this->assign('upgrade_url', (is_https()?'https://':'http://').trim($this->config('upgrade_url'),'/'));
