    ...
    public function index()
    {
        if (! ! $scode = request('scode', 'var')) {
            ...
            if (! ! $data = $this->model->getAbout($acode, $scode)) {
                ...
                $data->content = str_replace('//'.$this->config('main_domain').STATIC_DIR, '//'.$this->config('main_domain').STATIC_DIR.'#', $data->content);
                $data->content = str_replace(STATIC_DIR . '/upload/', get_http_url() . STATIC_DIR . '/upload/', $data->content);
                $data->content = str_replace(STATIC_DIR . '#/upload/', STATIC_DIR . '/upload/', $data->content);
                json(1, $data);
            } else {
